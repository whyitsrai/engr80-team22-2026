...
#define DELAY 0
const int diveDelay = 10000; // milliseconds to wait at each depth waypoint
...
void setup() {
  ...
  logger.include(&imu);
  logger.include(&gps);            // was commented out — GPS lat/lon/nsats now logged
  logger.include(&z_state_estimator); // added — logs filtered depth independently of depth_control
  ...
  logger.include(&as7262_sampler);
  logger.init();
  // burst_adc.init() intentionally omitted: burst_adc.sample() was never called in loop(),
  // so init() served only to call SD.begin() a second time. Re-enable both if burst
  // sampling is needed for a future mission.
  ...
  double depth_waypoints [] = { 0.5, 1, 1.5, 0 };  // listed as z0,z1,... etc.
  ...
  // burst_adc.lastExecutionTime not seeded — burst_adc is intentionally disabled this mission
  //Petting the dog
  //watchdog.feed();
}
