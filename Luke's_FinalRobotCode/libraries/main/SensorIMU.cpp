...
void SensorIMU::read(void) {
  ...
  // Get new data samples; bail out on I2C failure and keep last valid state.
  int32_t raw_acc_data[3];
  int32_t raw_mag_data[3];

  if (myIMU.Acc->GetAxes(raw_acc_data) != LSM303AGR_ACC_STATUS_OK ||
      myIMU.Mag->GetAxes(raw_mag_data) != LSM303AGR_MAG_STATUS_OK) {
    printer.printMessage("IMU read failed; holding last valid state", 5);
    return;
  }
  ...
  getOrientation(state.accelX,state.accelY,state.accelZ,state.magX,state.magY,state.magZ);
}
